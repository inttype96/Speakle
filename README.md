...
2. **Translation Model**  
   ...
   - 단, 해당 모델은 상업적 이용이 제한된 데이터셋을 재가공하여 사용하였으므로 **상업적 활용 시 유의가 필요합니다**.  
...
4. **Spotify Tracks Attributes and Popularity**  
   - License: 원 출처([ConquestAce/spotify-songs](https://huggingface.co/datasets/ConquestAce/spotify-songs))의 오리지널 라이선스 적용.  
   - Hugging Face 페이지 내 라이선스 미기재 → **보호된 데이터로 간주하고 활용에 주의해야 함**.  
   - 사용 전 반드시 원본 저장소의 라이선스 조건 확인 필요.  
...
- 모델 라이선스:  
  ...
  - 단, 상업적 이용 제한 데이터셋을 기반으로 한 산출물이 포함될 수 있으므로 활용 시 주의 필요.  
- 데이터셋 라이선스:  
  - **Apache 2.0 / CC0** → 상업적 활용 가능.  
  - **CC BY-NC-SA 4.0** → 비영리 목적에서만 활용 가능.  
  - **출처 기반 라이선스(ConquestAce/spotify-songs)** → 라이선스 명시 없음, 보호된 데이터로 간주. 반드시 원 저장소의 조건 확인 필요.  
- **중요 고지**: 라이선스가 불명확하거나 상업적 이용이 제한된 데이터셋(CC BY-NC-SA 4.0, 라이선스 미기재 데이터)은 **상업적 프로젝트에 포함할 수 없습니다**. 연구·실험적 목적에서만 활용하시기 바랍니다.  
